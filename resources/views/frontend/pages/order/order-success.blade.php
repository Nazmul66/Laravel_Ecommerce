...
@section('title')
   <title>Ecommerce | Order Summary</title>
@endsection
            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="product-order">
                      ...
                      @foreach ( $product_cart_details as $cart )
                        <div class="row product-order-detail">
                            <div class="col-3"><img src="{{ asset('frontend/assets/images/pro3/27.jpg') }}" alt="" class="img-fluid blur-up lazyload"></div>
                                ...
                                <div>
                                    ...
                                    <h5>
                                        @foreach ($product_details as $product_detail)
                                            @if ( $product_detail->id == $cart->product_id )
                      ...
                      @endforeach
                        ...
                        <div class="total-sec">
                            <ul>
                                <li>Subtotal <span>৳{{ $order_details->amount }} BDT</span></li>
                                <li>Shipping <span>৳00.00 BDT</span></li>
                                <li>Tax(GST) <span>৳00.00 BDT</span></li>
                            </ul>
                        </div>
                        <div class="final-total">
                            <h3>Total <span>৳{{ $order_details->amount }} BDT</span></h3>
                        </div>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                ...
                                <ul class="order-detail">
                                    ...
                                    <li>Order Date: {{ $order_details->created_at }}</li>
                                    <li>Order Total: ৳{{ $order_details->amount }} BDT</li>
                                </ul>
                            </div>
                            <div class="col-sm-6">
                                ...
                                <ul class="order-detail">
                                    ...
                                    <li>{{ $order_details->addressLine1 }}, {{ $order_details->addressLine2 }}</li>

                                    <li>
                                        @foreach ( App\Models\District::where('status', 1)->get() as $district)
                                            @if ( $district->id == $order_details->district_id )
                                                {{ $district->name }}
                                            @endif
                                        @endforeach, 

                                        @foreach ( App\Models\State::where('status', 1)->get() as $state)
                                            @if ( $state->id == $order_details->division_id )
                                                {{ $state->name }}
                                            @endif
                                        @endforeach
                                    </li>
                                    ...
                                    <li>
                                        @foreach ( App\Models\Country::where('status', 1)->get() as $country)
                                            @if ( $country->id == $order_details->country )
                                                {{ $country->name }}
                                            @endif
                                        @endforeach, 
                                    ...
                                    <li>Contact No. {{ $order_details->phone }}</li>
                                </ul>
                            </div>
                            ...
                            <div class="col-md-12">
                                <div class="delivery-sec">
                                    <h3>expected date of delivery: <span>{{ $expected_date_time }}</span></h3>
